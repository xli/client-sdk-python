...
class InMemory:
    ...
    def create_event(self, account_id: str, type: str, data: str) -> None:
        self.create(Event, account_id=account_id, type=type, data=data, timestamp=_ts())

    def find(self, typ: Type[T], **conds: Any) -> T:
        ret = next(self._select(typ, **conds), None)
        if ret is None:
