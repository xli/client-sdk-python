...
from .models import Transaction, ReceivePayment, Command
...
class EventPuller:
    ...
    def save_payment_txn(self, event: jsonrpc.Event) -> None:
        ...
        if isinstance(metadata, diem_types.GeneralMetadataV0) and metadata.to_subaddress:
            res = self.store.find(ReceivePayment, subaddress_hex=metadata.to_subaddress.hex())
        ...
        if isinstance(metadata, diem_types.TravelRuleMetadataV0) and metadata.off_chain_reference_id:
            cmd = self.store.find(Command, last=True, reference_id=metadata.off_chain_reference_id)
    ...
    def _create_txn(self, event: jsonrpc.Event, **kwargs) -> None:  # pyre-ignore
        self.store.create(
            Transaction,
