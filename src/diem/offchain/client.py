...
class Client:
    ...
    def process_inbound_request(self, request_sender_address: str, request_bytes: bytes) -> Command:
        ...
        if request.command_type == CommandType.PaymentCommand:
            ...
            cmd = self.create_payment_command(request.cid, payment)
            if cmd.is_initial():
    ...
    def create_inbound_payment_command(self, cid: str, obj: PaymentObject) -> PaymentCommand:
        return self.create_payment_command(cid, obj)

    def create_payment_command(self, cid: str, obj: PaymentObject) -> PaymentCommand:
        if self.is_my_account_id(obj.sender.address):
            return PaymentCommand(cid=cid, my_actor_address=obj.sender.address, payment=obj)
        if self.is_my_account_id(obj.receiver.address):
            return PaymentCommand(cid=cid, my_actor_address=obj.receiver.address, payment=obj)
